...
const Indicator = memo(({ progress, index, size, count, color }) => {
...
const ActivityIndicatorComponent = ({ size: customSize, count: customCount, color: customColor, }) => {
    ...
    const color = customColor !== null && customColor !== void 0 ? customColor : theme.componentTheme.activityIndicator.color;
    useEffect(() => {
    ...
    return (<View style={{
      ...
      {range(0, count).map(index => (<Indicator key={index} progress={progress} index={index} size={size} count={count} color={color}/>))}
    </View>);
};
